...
#   python test_release.py --short ~/nta/current ~/nta/trunk --test=hypersearch_test
#
# ----------------------------------------------------------------------
...
################################################################################
class ModelChooserTests(ExperimentTestBaseClass):
  ...
  #############################################################################
  def setUp(self):
    OPFDummyModelRunner.modelIndex = 0
    self.expDir = os.path.join(g_myEnv.testSrcExpDir, 
                               'dummyV%d' % g_myEnv.options.hsVersion)
    self.hsImp = "v%s" % g_myEnv.options.hsVersion

    self.env = {'NTA_CONF_PROP_nupic_hypersearch_enableModelTermination':'0',
                'NTA_CONF_PROP_nupic_hypersearch_enableModelMaturity':'0',
                'NTA_CONF_PROP_nupic_hypersearch_enableSwarmTermination':'0'}


  ############################################################################
  def assertBestModel(self, jobID, bestModelID, bestValue, allModelIDs,
                      outputCacheModels):
    """
    Asserts that the best model in the jobs record is equal to bestModelID,
    and that the only output cache that exists is the one for the best model

    Parameters:
    -----------------------------------------------------------------------
    jobID:                ID of the job for hypersearch job
    bestModelID:          The expected modelID of the best model
    bestValue:            The expected metric value of the best model
    allModelIDs:          A list of all the model IDs for the job
    outputCacheModels:    A list of modelIDs that should have output caches
    """

    cjDB = ClientJobsDAO.get()
    results = json.loads(cjDB.jobGetFields(jobID, ['results'])[0])
    self.assertEqual(results['bestModel'], bestModelID)
    self.assertAlmostEqual(results['bestValue'], bestValue, places=4)

    self.assertEqual(len(results['metrics']), 1)
    self.assertAlmostEqual(results['metrics'].values()[0],
                           results['bestValue'], places=4)

    # -----------------------------------------------------------------------
    # Garbage Collect
    # -----------------------------------------------------------------------
    hsgc = HSGarbageCollector()
    hsgc.run()

    # -----------------------------------------------------------------------
    # Assert the appropriate output caches exist
    # -----------------------------------------------------------------------
    dsMgr = DataStreamsMgr.get()
    
    debugInfo = (
      "jobID=%s; bestModelID=%s; outputCacheModels=%s; allModelIDs=%s" % (
        jobID, bestModelID, outputCacheModels, allModelIDs))
    for id in allModelIDs:
      streamID = "model-%s" % id
      if id in outputCacheModels:
        self.assertTrue(dsMgr.doesStreamExist(streamID),
                        "Expected output cache '%s' was not found; %s" % (
                          streamID, debugInfo))
      else:
        self.assertFalse(dsMgr.doesStreamExist(streamID),
                        "Unexpected output cache '%s' was found; %s" % (
                          streamID, debugInfo))



  ############################################################################
  def testBasicBestFirst(self):
    """
    Try running a simple test with dummy model. In this test, the best model
    starts running before the other model
    """
    
    # NOTE: save stream names before running this swarm to help debug
    #  NUP-1637 "Unexpected model output file during hypersearch_test"
    self.addExtraLogItem(
      "All streams before swarms: %s" % (DataStreamsMgr.listAllStreams(),))


    self._printTestHeader()
    dummyParams = {'metricFunctions':
                    ['lambda x: 35',
                      'lambda x: 40'],
                  'iterations':100,
    ...
    # =========================================================================
    # Test 1: Don't clean up output caches
    # =========================================================================
    dummyParams['finalize'] = False
    jobID,_,_,_,_ = self.runPermutations(self.expDir, hsImp=self.hsImp,
                                         maxModels=2,
                                         useStreams=True,
                                         loggingLevel = g_myEnv.options.logLevel,
                                          env=self.env,
                                          dummyModel=dummyParams)


    cjDB = ClientJobsDAO.get()
    modelIDs = sorted(cjDB.jobGetModelIDs(jobID))
    
    self.assertBestModel(jobID, bestModelID=modelIDs[0],
                                bestValue=35,
                                allModelIDs=modelIDs,
                                outputCacheModels=modelIDs[0:1])

    results = json.loads(cjDB.jobGetFields(jobID, ['results'])[0])
    self.assertEqual(results['saved'], False)


    # =========================================================================
    # Test 2: Clean up output caches
    # =========================================================================
    dummyParams['finalize'] = True
    jobID,_,_,_,_ = self.runPermutations(self.expDir, hsImp=self.hsImp,
                                         maxModels=2,
                                         useStreams=True,
                                         loggingLevel = g_myEnv.options.logLevel,
  ...
  ############################################################################
  def testBasicBestLast(self):
    """
    Try running a simple test with dummy model. In this test, the best model
    starts running after the other model
    """
    ...
    cjDB = ClientJobsDAO.get()
    modelIDs = sorted(cjDB.jobGetModelIDs(jobID))
    self.assertBestModel(jobID, bestModelID=modelIDs[1],
                                bestValue=35,
                                allModelIDs=modelIDs,
                                outputCacheModels=modelIDs[1:])

  ############################################################################
  def testOrder(self):
    """
    Run a test with more complex metric functions in an arbitrary order
    """
    self._printTestHeader()
    metrics = ['lambda x: float(x)',
               'lambda x: 100 - (x/9.5)**2',
               'lambda x: ((x-50)**2)/8 - 50']

    for metricsList in itertools.permutations(metrics):
      self.setUp()
      jobID,_,_,_,_ = self.runPermutations(self.expDir, hsImp=self.hsImp,
                                           maxModels=len(metrics),
                                            env=self.env,
                                            useStreams=True,
                                            loggingLevel = g_myEnv.options.logLevel,
                                            dummyModel={'metricFunctions': metricsList,
                                                        'experimentDirectory':self.expDir,
                                                        'iterations':100,
                                           })
      ...
      targetIndex = metricsList.index('lambda x: 100 - (x/9.5)**2')
      targetVal = eval(metrics[1])(100)
      self.assertBestModel(jobID, bestModelID=modelIDs[targetIndex],
                                bestValue=targetVal,
                                allModelIDs=modelIDs,
                                outputCacheModels=[modelIDs[targetIndex]])

################################################################################
class ModelChooserMNTests(ExperimentTestBaseClass):
  """
  """
  # AWS tests attribute required for tagging via automatic test discovery via
  # nosetests
  ...
  def setUp(self):
    ...
    hsVersion = 2
    self.expDir = os.path.join(g_myEnv.testSrcExpDir,'dummy_multi_v%d' % hsVersion)

    self.hsImp = "v%s" % hsVersion
  ...
  ############################################################################
  def assertBestModel(self, jobID, modelNum, targetValue,
                      allModelNums, cachedModelNums):

    cjDB = ClientJobsDAO.get()
    modelIDs = cjDB.jobGetModelIDs(jobID)

    modelParams = cjDB.modelsGetFields(modelIDs, ['params'])
    modelIDs = [e[0] for e in modelParams]
    modelOrders = [json.loads(e[1][0])['structuredParams']['__model_num']
                    for e in modelParams]

    modelOrderMap = dict(zip(modelOrders, modelIDs))

    bestIDTarget = modelOrderMap[modelNum]

    results = json.loads(cjDB.jobGetFields(jobID, ['results'])[0])
    self.assertEqual(bestIDTarget, results['bestModel'],
                    "Best model=%s, Expected=%s" %(results['bestModel'],
                                                   bestIDTarget))

    self.assertAlmostEqual(targetValue, results['bestValue'],
                           "Best error is not correct. "
                          "Target=%f Actual=%f"% (targetValue, results['bestValue']))
    ...
    for num in allModelNums:
      modelID = modelOrderMap[num]
      streamID = "model-%s" % modelID
      if num in cachedModelNums:
        self.assertTrue(dsMgr.doesStreamExist(streamID),
                        "Expected output cache '%s' was not found" % streamID)
      else:
        self.assertFalse(dsMgr.doesStreamExist(streamID),
                        "Unexpected output cache '%s' was found" % streamID)
  ...
  def testBasicBestFirst(self):
    ...
    self.env.update({'NTA_TEST_max_num_models':'2'})
    ...
    cjDB = ClientJobsDAO.get()
    self.assertBestModel(jobID, modelNum=0,
                                targetValue=100,
                                allModelNums=range(2),
                                cachedModelNums=[0])
  ...
  def testBasicBestLast(self):
    ...
    cjDB = ClientJobsDAO.get()
    modelIDs = cjDB.jobGetModelIDs(jobID)
    self.assertBestModel(jobID, modelNum=1,
                                targetValue=35,
                                allModelNums=range(2),
                                cachedModelNums=[1])
  ...
  def testInterleaved(self):
    ...
    metrics = ['lambda x: x', 'lambda x: 100-x']

    dummyParams['delay'] = [0.0, 2.0]
    dummyParams['finalDelay'] = [4.0, 0.0]
    for metricList in itertools.permutations(metrics):
      dummyParams['metricFunctions'] = metricList
      jobID,_,_,_,_ = self.runPermutations(self.expDir, hsImp=self.hsImp,
                                           maxModels=2,
                                           useStreams=True,
                                           onCluster=True,
      ...
      targetModelNum = metricList.index(metrics[1])
      targetVal = eval(metrics[1])(iterations)

      self.assertBestModel(jobID, modelNum=targetModelNum,
                                  targetValue=targetVal,
                                  allModelNums=range(2),
                                  cachedModelNums=[targetModelNum])

      results = json.loads(cjDB.jobGetFields(jobID, ['results'])[0])
      self.assertEqual(results['saved'], True)

  ############################################################################
  def testBestThenKilled(self):
    """
    This test checks to make sure that if the "best model" dies while it is
    running (due to error or node failure), The best model is still correct
    """


    self._printTestHeader()
    self.env.update({'NTA_TEST_max_num_models':'2',
                    "NTA_CONF_PROP_nupic_hypersearch_modelOrphanIntervalSecs": '10'})

    dummyModelParams ={'metricFunctions':
                        ['lambda x: x',
                         'lambda x: 0.5*x + 10'],
                      'experimentDirectory':self.expDir,
                      'delay':[0, 2],
    ...
    params = dict(expDirectory=self.expDir, hsImp=self.hsImp,
                  maxModels=2,
                  onCluster=True,
                  useStreams=True,
                  loggingLevel=g_myEnv.options.logLevel,
                  env=self.env,
                  maxNumWorkers=4,
                  dummyModel=dummyModelParams)

    dummyModelParams['iterations'] = 100
    dummyModelParams['finalize'] = True
    jobID, _,_,_,_ = self.runPermutations(**params)

    targetVal = eval(dummyModelParams['metricFunctions'][1])(100)

    self.assertBestModel(jobID=jobID, modelNum=1,targetValue=targetVal,
                         allModelNums=range(2), cachedModelNums=[1])
  ...
  ############################################################################
  def assertKilledModels(self, jobID, killedModelNums, targetRecords):
    cjDB = ClientJobsDAO.get()
    modelInfo  = self.getModelFields(jobID=jobID, fields=['numRecords',
                                                          'completionReason'])

    for i in xrange(len(modelInfo)):
      if i in killedModelNums:
        self.assertEqual(modelInfo[i][2] , cjDB.CMPL_REASON_KILLED)
      else:
        self.assertEqual(modelInfo[i][2] , cjDB.CMPL_REASON_EOF)

      self.assertEqual(modelInfo[i][1], targetRecords[i])
...
################################################################################
class ModelCheckpointTests(ExperimentTestBaseClass):
  """
  """
  # AWS tests attribute required for tagging via automatic test discovery via
  # nosetests
  ...
  def setUp(self):
    ...
    self.env = {'NTA_CONF_PROP_nupic_hypersearch_enableModelTermination':'0',
                'NTA_CONF_PROP_nupic_hypersearch_enableModelMaturiy':'1',
                'NTA_CONF_PROP_nupic_hypersearch_maturityMaxSlope':'0.1',
                'NTA_CONF_PROP_nupic_hypersearch_enableSwarmTermination':'0',
                'NTA_CONF_PROP_nupic_hypersearch_bestModelMinRecords':'0'}
  ...
  ############################################################################
  def assertBestModel(self, jobID, modelNum, targetValue,
                      allModelNums, cachedModelNums, checkpointedModelNums):

    cjDB = ClientJobsDAO.get()
    modelIDs = cjDB.jobGetModelIDs(jobID)

    modelParams = cjDB.modelsGetFields(modelIDs, ['params', 'modelCheckpointId'])
    modelIDs = [e[0] for e in modelParams]
    modelOrders = [json.loads(e[1][0])['structuredParams']['__model_num']
                    for e in modelParams]
    modelCheckpoints = [e[1][1] for e in modelParams]

    modelOrderMap = dict(zip(modelOrders, modelIDs))
    modelCheckpointMap = dict(zip(modelOrders, modelCheckpoints))
    ...
    # -----------------------------------------------------------------------
    # Assert that the appropriate models are checkpointed
    # -----------------------------------------------------------------------
    for num in allModelNums:
      if num in checkpointedModelNums:
        self.assertFalse(modelCheckpointMap[num] is None,
                         "Expected checkpoint for model %s not found"
                         % modelOrderMap[num])
      else:
        self.assertTrue(modelCheckpointMap[num] is None,
                         "Unexpected checkpoint for model %s found"
                         % modelOrderMap[num])
  ...
  def testSimpleMN(self):
    """ Simple test to make sure that a checkpoint gets created when the best
    model completes"""
    ...
    targetValue = eval("lambda x: -10*math.log10(x+1) +100")(350)
    self.assertBestModel(jobID, modelNum=0, targetValue=targetValue,
                         allModelNums=range(2),
                         cachedModelNums=[0],
                         checkpointedModelNums=[0])

  ############################################################################
  def testFewRecords(self):
    self._printTestHeader()
    self.env['NTA_TEST_max_num_models'] = '2'
    jobID,_,_,_,_ = self.runPermutations(self.expDir, hsImp=self.hsImp, maxModels=2,
                                loggingLevel = g_myEnv.options.logLevel,
    ...
    targetValue = eval("lambda x: -10*math.log10(x+1) +100")(10)
    self.assertBestModel(jobID, modelNum=0,
  ...
  ############################################################################
  def testSorted(self):
    """This is a test to make sure that, when a model reaches the end, it
    checkpoints itself IF AND ONLY IF it is better than the best checkpointed
    model

    The test runs 5 models. Model 5 is the global best, and runs after all the
    other models. The models that finish shouldn't compare themselves to this
    model when deciding to checkpoint

    * Models 1, 2 should complete in that order and should then be checkpointed
    * Model 3 will then complete, but then shouldn't be checkpointed because it
      is not the best
    * Model 4 should then finish and be checkpointed
    """

    self._printTestHeader()
    self.env['NTA_TEST_max_num_models'] = '5'
    jobID,_,_,_,_ = self.runPermutations(self.expDir, hsImp=self.hsImp,
                                       maxModels=5,
                                       loggingLevel = g_myEnv.options.logLevel,
                                       env = self.env,
    ...
    self.assertBestModel(jobID, modelNum=4, targetValue=10,
                         allModelNums=range(5), cachedModelNums=[4],
                         checkpointedModelNums=[4])


  ############################################################################
  def testBestNotSaved(self):
    """ In this case, model 2 is the best model right until the very end.
    Make sure that model 1 is still checkpointed, as it ends up being the best"""
    ...
    self.assertBestModel(jobID, modelNum=0, targetValue=100,
                         allModelNums=range(2), cachedModelNums=[0],
                         checkpointedModelNums=[0])
...
############################################################################
class HypersearchWithParamTests(ExperimentTestBaseClass):
  """
  """
  # AWS tests attribute required for tagging via automatic test discovery via
  # nosetests
  ...
  def setUp(self):
    super(HypersearchWithParamTests, self).setUp()

  def _testHypersearchJobWithPredictionCacheParam(self, maxRecords,
                                                  predictionCacheMaxRecords=None,
                                                  timeoutSec=None):
    (jobID,_,_,_,_) \
      =  self.runPermutations(expDirectory=None, hsImp='v2',
           maxModels=1,
           useStreams=True, onCluster=True,
           waitForCompletion=True,
           maxRecords=maxRecords,
           dataPath="extra/hotgym/hotgym.csv",
           timeoutSec=timeoutSec,
           predictionCacheMaxRecords=predictionCacheMaxRecords)

    modelID = getHypersearchWinningModelID(jobID)

    self.assertNotEqual(modelID, None)

    print "ModelID %r of Hypersearch JobID %r won" % (
      modelID, jobID)

    numPredictionRecords = DataStreamsMgr.get().getStreamSize(
      PredictionOutputStream.getStreamIDForModelID(modelID))

    if predictionCacheMaxRecords is None or maxRecords < predictionCacheMaxRecords:
      numExpectedPredictionRecords = maxRecords
    else:
      numExpectedPredictionRecords = predictionCacheMaxRecords

    self.assertEqual(
      numPredictionRecords, numExpectedPredictionRecords,
      msg=("Unexpected output record count; Winning Model %r of Job %r; "
           "numPredictionRecords=%r; "
           "numExpectedPredictionRecord=%r; ") % (
        modelID, jobID,
        numPredictionRecords,
        numExpectedPredictionRecords))

  def testHypersearchJobWithPredictionCacheParamNone(self):
    self._testHypersearchJobWithPredictionCacheParam(maxRecords=1000)

  def testHypersearchJobWithPredictionCacheParamSmall(self):
    self._testHypersearchJobWithPredictionCacheParam(maxRecords=1000, predictionCacheMaxRecords=55)

  #
  # This test the prediction cache handling in Hypersearch Job to manage the sliding window that
  # contains more records then the default specification of 10000 records.
  #
#  def testHypersearchJobWithPredictionCacheParamLarge(self):
#    self._testHypersearchJobWithPredictionCacheParam(maxRecords=15000, predictionCacheMaxRecords=12000, timeoutSec=600)



class HypersearchAPITests(ExperimentTestBaseClass):
  """Test hypersearch API functions"""
  ...
  def testGetSwarmModelParams(self):
    (jobID, jobInfo, resultsInfoForAllModels, metricResults, minErrScore) = \
      self.runPermutations(expDirectory=None, maxModels=1, maxNumWorkers=1,
                           useStreams=True, onCluster=True, maxRecords=50)
    bestModelID = json.loads(jobInfo.results)["bestModel"]
    
    modelParams = json.loads(getSwarmModelParams(bestModelID))
...
def _getTestList():
  ...
  suiteNames = [
                'OneNodeTests', 'MultiNodeTests',
                'ModelChooserTests',
                'ModelChooserMNTests', 'ModelMaturityTests',
                'ModelCheckpointTests', 'SwarmTerminatorTests',
                'HypersearchWithParamTests',
               ]
