...
class MySPRegion(PyRegion):
  ...
  """
  MySPRegion is designed to implement the spatial pooler compute for a given
  HTM level.
  ...
  If you add a parameter to SpatialPooler, it will be exposed through MySPRegion
  automatically as if it were in MySPRegion.__init__, with the right default
  value. Add an entry in the __init__ docstring for it too, and that will be
  brought into the NodeSpec. MySPRegion will maintain the parameter as its own
  instance variable and also pass it to SpatialPooler. If the parameter is
  changed, MySPRegion will propagate the change.

  If you want to do something different with the parameter, add it as an
  argument into MySPRegion.__init__, which will override all the default handling.
  """
  ...
  def compute(self, inputs, outputs):
    """
    Run one iteration of MySPRegion's compute, profiling it if requested.
  ...
  @classmethod
  def getSpec(cls):
    """Return the Spec for MySPRegion.
