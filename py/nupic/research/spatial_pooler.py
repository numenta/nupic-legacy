...
# change boosting implementations here
from nupic.research.boosting.boosting_simple import BoostingSimple as Boosting
...
class SpatialPooler(object):
  ...
  def __init__(self,
    ...
    self._boost=Boosting(self, minPctOverlapDutyCycle,minPctActiveDutyCycle,dutyCyclePeriod,maxBoost)
  ...
  def compute(self, inputVector, learn, activeArray):
    ...
    if learn:
      boostedOverlaps = self._boost.doBoosting(overlaps)
    else:
  ...
  def _bumpUpWeakColumns(self):
    ...
    """
    weakColumns = numpy.where(self._boost._overlapDutyCycles
                                < self._boost._minOverlapDutyCycles)[0]
