...
class SpatialPooler(object):
  ...
  def __init__(self,
    ...
    columnDimensions:     A number, list or numpy array representing the 
                          ...
                          of one dimesion with 2000 columns use 2000, or [2000].
                          For a three dimensional topology of 32x64x16 use 
                          [32, 64, 16]. 
    potentialRadius:      This parameter deteremines the extent of the input 
                          ...
                          in the 'global coverage', meaning that each column can 
                          potentially be connected to every input bit. This 
                          parameter defines a square (or hyper square) area: a 
                          column will have a max square potential pool with 
                          sides of length 2 * potentialRadius + 1. 
    potentialPct:         The percent of the inputs, within a column's potential 
                          radius, that a column can be connected to. If set to 
                          1, the column will be connected to every input within 
                          its potential radius. This parameter is used to give 
                          each column a unique potential pool when a large 
                          potentialRadius causes overlap between the columns.
                          At initialization time we choose (2*potentialRadius 
                          + 1)^(# inputDimensions) * potentialPct input bits to 
                          comprise the receptive field of the column.
    ...
    localAreaDensity:     The desired density of active columns within a local
                          ...
                          potential pools of all columns). The inhibition logic
    ...
    stimulusThreshold:    This is a number specifying the minimum number of
                          ...
                          turn ON. The purpose of this is to prevent noise input 
                          from activating columns. Specified as a percent of a
                          fully grown synapse.
    synPermInactiveDec:   The amount by which an inactive synapse is decremented 
                          in each round. Specified as a percent of a fully grown 
                          synapse.
    synPermActiveInc:     The amount by which an active synapse is incremented 
                          in each round. Specified as a percent of a
                          fully grown synapse.
    synPermActiveSharedDec: The amount by which to decrease the permanence of an 
                          active synapse which is connected to another column 
                          that is active at the same time. Specified as a 
                          percent of a fully grown synapse.
    synPermOrphanDec:     The amount by which to decrease the permanence of an 
                          active synapse on a column which has high overlap with 
                          the input, but was inhibited (an "orphan" column).
    ...
    minPctOvlerapDutyCycle: A number between 0 and 1.0, used to set a floor on
                          ...
                          On each iteration, any column whose overlap duty cycle 
                          falls below this computed value will  get
    ...
    dutyCyclePeriod:      The period used to calculate duty cycles. Higher
     ...
     maxBoost:            The maximum overlap boost factor. Each column's
                          ...
                          1.0 and maxBoost. A boost factor of 1.0 is used
                          if the duty cycle is >= minOverlapDutyCycle, maxBoost
    ...
    # connected to inputs in its potential poolThe indices refer to a falttenned 
    # version of both the inputs and columns. Namely, irrespective of the 
    # topology of the inputs and columns, they are treated as being a one 
    # dimensional array. Since a column is typically connected to only a subset
    # of the inputs, many of the entries in the matrix are 0. Therefore the 
    # the potentialPool matrix is stored using the SparseBinaryMatrix class, 
    # to reduce memory footprint and compuation time of algorithms that require
    # iterating over the data strcuture.
    ...
    # represent the cortial columns, and whose columns represent the input bits.
    # if self._permanences[i][j] = 0.2, then the synapse connecting cortical 
    # column 'i' to input bit 'j'  has a permanence of 0.2. Here we also use
    # the SparseMatrix class to reduce the memory footprint and computation time
    # of algorithms that require iterating over the data structure. 
    self._permanences = SparseMatrix(numColumns,numInputs)

    # 'self._connectedSynapses' is a similar matrix to 'self._permanences' (rows 
    # represent cortial columns, columns represent input bits) whose entries 
    # represent whether the cortial column is connected to the input bit, i.e. 
    # its permanence value is greater than 'synPermConnected'. While this 
    # information is readily available from the 'self._permanence' matrix, it is 
    ...
    # Ensure that each column is connected to enough input bits such that it has
    # a chance of being activated.
  ...
  def _calculateAnomalyScore(self, overlaps, activeColumns):
    ...
    similarity between the current input, and inputs that have activated them in 
    the past. Furthermore, since some columns become active more frequently than 
    others, having an infrequently active column become active is another 
  ...
  def _updateInhibitionRadius(self):
    ...
    exist for each input. For multiple dimension the aforementioned calculations
    are averaged over all dimensions of inputs and columns. This value is 
    meaningless if global inhibition is enabled.
    """
  ...
  def _avgColumnsPerInput(self):
    """
    The average number of columns per input, taking into account the topology of
    the inputs and columns. This value is used to calculate the inhibition 
  ...
  def _avgConnectedSpanForColumn2D(self,index):
    """
    The range of connectedSynapses per column, averaged for each dimension. This 
    vaule is used to calculate the inhibition radius. This variation of the 
    function only supports a 2 dimensional column topology.
    """
  ...
  def _avgConnectedSpanForColumnND(self,index):
    ...
    The range of connectedSynapses per column, averaged for each dimension. This 
    vaule is used to calculate the inhibition radius. This variation of the 
    function only supports arbitrary column dimensions.
    """
  ...
  def _adaptSynapses(self,inputVector,sharedInputs,orphanColumns):
    ...
    columns with 100 percent overlap, are treated slightly differently.
    """
  ...
  def _mapPotential(self,index):
    """
    Maps a column to its input bits. This method encapsultes the topology of the
    ...
    * If the potentialRadius is greater than or equal to the entire input space,
      (global visibility), then this method returns an array filled with all 
      the indices
  ...
  def _calculateOverlap(self,inputVector):
    ...
    to input bits which are turned on. The implementation takes advandage of the
  ...
  def _calculateOrphanColumns(self,activeColumns, overlapsPct):
    """
    Determine the orphan columns for a given set of active columns and overlaps.
    Orphan columns are defined as columns with 100% overlap of the input vector,
    meaning each one of their connected synapses was connected to an input bit
    which was turned on, yet the columns did not survive the inhibition round.
    Essentially these are columns who have learned a particular input pattern, 
    but there  exists other columns who have learned a better representation of 
    that same pattern.
    """
  ...
  def _getNeighbors2D(columnIndex, dimensions, radius):
    ...
    it in each dimension. The method returns a list of the flat indices of these
    columns. This method is a subset of _getNeighborsND and is only included for 
    illustration purposes, and potentially enhanced performance for spatial 
    pooler implementations that only require a one-dimensional topology.
    """
  ...
  def _getNeighborsND(columnIndex, dimensions, radius):
    """
    Similar to _getNeighbors1D and _getNeighbors2D, this function Returns a list 
    of indices corresponding to the neighbors of a given column. Since the 
    permanence values are stored in such a way that information about toplogy is 
    lost. This method allows for reconstructing the toplogy of the inputs, which 
    are flattened to one array. Given a column's index, its neighbors are defined 
    as those columns that are 'radius' indices away from it in each dimension. 
    The method returns a list of the flat indices of these columns. 
    """
