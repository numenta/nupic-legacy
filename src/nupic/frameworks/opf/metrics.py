...
AAE is average absolute error

altMAPE is mean absolute percentage error but modified so you never have divide by zero
...
class MetricSpec(object):
  ...
  def getInferenceTypeFromLabel(cls, label):
    ...
    Parameters:
    -----------------------------------------------------------------------
    :param label:      A label (string) for a metric spec generated by getMetricLabel
                (above)
...
def getModule(metricSpec):
  """
      ...
      :param metricSpec: - an instance of MetricSpec.
      :param metricSpec.metric: must be one of:
...
class AggregateMetric(MetricsIface):
  ...
  def aggregate(self, accumulatedError, historyBuffer, steps):
    ...
    :param steps: The total number of (groundTruth, prediction) pairs that have
