...
class ValueSensor(PyRegion):
  """
  A ValueSensor holds a value and encodes it into network output.
  """
  ...
  def getSpec(cls):
    return {
      'singleNodeOnly': True,
      'description': ValueSensor.__doc__,
      'outputs': {
        'encoded': {
          'description': '',
          'dataType': 'Real32',
  ...
  def getParameter(self, parameterName, index=-1):
    if parameter == 'sensedValue':
      raise Exception('For the ValueSensor, get the sensedValue via the getSensedValue method')
    else:
      raise Exception('Unrecognized parameter %s' % parameterName)

  def setParameter(self, parameterName, index, parameterValue):
    if parameter == 'sensedValue':
      raise Exception('For the ValueSensor, set the sensedValue via the setSensedValue method')
    else:
      raise Exception('Unrecognized parameter %s' % parameterName)
